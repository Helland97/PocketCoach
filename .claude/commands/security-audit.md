...
2. Build the prompt:
   ...
   - If `$ARGUMENTS` starts with `execute`: pass it through as `"mode: execute, items: <rest of args>. Re-read the relevant files first, apply only the listed items, then write SECURITY-AUDIT.local.md per your instructions."`
...
5. After an execute run, do not commit or push — the agent already declines to do so. Tell the user to review `git status` and the new `SECURITY-AUDIT.local.md` themselves.
